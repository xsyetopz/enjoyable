...
public actor USBDeviceManager {
  ...
  private var _usbWatchdog: USBWatchdog?
  private var _quirkManager: QuirkManager?
  private let _eventHandler: @Sendable (USBDeviceEvent) -> Void
  
  public init(
    adapter: LibUSBAdapter,
    watchdog: USBWatchdog? = nil,
    quirkManager: QuirkManager? = nil,
    eventHandler: @escaping @Sendable (USBDeviceEvent) -> Void = { _ in }
  ) {
    self._adapter = adapter
    self._configMatcher = ConfigurationMatcher()
    self._usbWatchdog = watchdog
    self._quirkManager = quirkManager
    self._eventHandler = eventHandler
  }
  
  public func setWatchdog(_ watchdog: USBWatchdog?) {
    _usbWatchdog = watchdog
  }
  
  public func setQuirkManager(_ manager: QuirkManager?) {
    _quirkManager = manager
  }
  ...
  public func stop() async {
    ...
    for deviceID in _connectedDevices.keys {
      await _stopMonitoringDevice(deviceID: deviceID)
    }
  }
  ...
  public func connect(deviceID: Core.USBDeviceID) async throws -> GamepadDevice {
    ...
    if let config = _configMatcher.bestConfiguration(
      ...
      await _applyQuirks(for: deviceID, configuration: config)
    }
  ...
  private func _connectDevice(deviceID: Core.USBDeviceID, detectedDevice: GamepadDevice) async {
    do {
      ...
      await _startMonitoringDevice(deviceID: deviceID)
    } catch {
  ...
  private func _applyQuirks(for deviceID: Core.USBDeviceID, configuration: DeviceConfiguration) async {
    guard let quirkManager = _quirkManager else {
      return
    }
    
    do {
      try await quirkManager.applyQuirks(for: deviceID, configuration: configuration)
    } catch {
      let event = USBDeviceEvent(
        type: .error,
        device: _connectedDevices[deviceID],
        error: error
      )
      _eventHandler(event)
    }
  ...
  private func _startMonitoringDevice(deviceID: Core.USBDeviceID) async {
    guard let watchdog = _usbWatchdog else {
      return
    }
    
    await watchdog.startMonitoring(deviceID: deviceID) { [weak self] deviceID in
      await self?._reconnectDevice(deviceID: deviceID)
    }
  }
  
  private func _stopMonitoringDevice(deviceID: Core.USBDeviceID) async {
    guard let watchdog = _usbWatchdog else {
      return
    }
    
    await watchdog.stopMonitoring(deviceID: deviceID)
  }
  
  private func _reconnectDevice(deviceID: Core.USBDeviceID) async {
    await _disconnectDevice(deviceID: deviceID)
    ...
    do {
      ...
      if let config = _deviceConfigurations[deviceID] {
        await _applyQuirks(for: deviceID, configuration: config)
      }
  ...
  private func _disconnectDevice(deviceID: Core.USBDeviceID) async {
    ...
    if let quirkManager = _quirkManager {
      await quirkManager.removeQuirks(for: deviceID)
    }
  ...
  private func _shouldStartKeepalive(config: DeviceConfiguration) -> Bool {
    return config.hasQuirk(named: "keepalive") && !config.hasQuirk(named: "no_keepalive")
  }
  ...
  private func _continuousReadLoop(deviceID: Core.USBDeviceID) async {
    ...
    while !Task.isCancelled {
      do {
        ...
        if let watchdog = _usbWatchdog {
          await watchdog.recordActivity(deviceID: deviceID)
        }
  ...
  private func _handleReadError(deviceID: Core.USBDeviceID, error: any Error) async {
    if let watchdog = _usbWatchdog {
      await watchdog.recordStall(deviceID: deviceID)
    }
